...
float adc_val_Vpv(uint16_t adc_result)                  // Use Voltage sensor Va for this from the card;
{
    float c_Vpv = 1.4641f;
    float m_Vpv = 0.0018901f;

    float value;

    float adc_val = (adc_result*3.0f)/4095.0f;

    value = (adc_val - c_Vpv)/m_Vpv;

    return value;

}

float adc_val_Vo(uint16_t adc_result)                   // Use Voltage sensor Vb for this from the card;
{
    float c_Vo = 1.4608f;
    float m_Vo = 0.00186f;
